...
--
-- Técnica:
-- COUNT FILTER + agregaciones condicionales
-- =============================================================================

SELECT
    dc.sexo,
    dc.edad_descripcion,
    dc.aliento_alcoholico,
    dc.cinturon_seguridad,
    SUM(fa.num_accidentes) AS total_accidentes,
    SUM(fa.total_muertos) AS total_muertos,
    SUM(fa.total_heridos) AS total_heridos,
    COUNT(*) FILTER (
        WHERE fa.total_muertos > 0 OR fa.total_heridos > 0
    ) AS accidentes_con_victimas,
    ROUND(
        100.0 * COUNT(*) FILTER (
            WHERE fa.total_muertos > 0 OR fa.total_heridos > 0
        ) / NULLIF(COUNT(*), 0),
        2
    ) AS porcentaje_con_victimas
FROM fact_accidentes fa
JOIN dim_conductor dc
    ON fa.conductor_key = dc.conductor_key
GROUP BY
    dc.sexo,
    dc.edad_descripcion,
    dc.aliento_alcoholico,
    dc.cinturon_seguridad
HAVING SUM(fa.num_accidentes) >= 100
ORDER BY accidentes_con_victimas DESC
LIMIT 20;
